...
{
    public partial class FrmOrdemEnvase : Form
    {
        string pasta_botoes = "";
        Image imagem_normal;
        Image imagem_mouse;
        ...
        {
            for (int i = 0; i < listOrdEn.Items.Count; i++)
            {
                listOrdEn.ItemSelectionChanged -= listOrdEn_ItemSelectionChanged;
                listOrdEn.ItemCheck -= listOrdEn_ItemCheck;
                listOrdEn.Items[i].Selected = listOrdEn.Items[i].Checked;
                listOrdEn.ItemSelectionChanged += listOrdEn_ItemSelectionChanged;
                listOrdEn.ItemCheck += listOrdEn_ItemCheck;
            }
        }
        private void listOrdEn_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            for (int i = 0; i < listOrdEn.Items.Count; i++)
            {
                listOrdEn.ItemChecked -= listOrdEn_ItemChecked;
                listOrdEn.ItemCheck -= listOrdEn_ItemCheck;
                listOrdEn.Items[i].Checked = listOrdEn.Items[i].Selected;
                listOrdEn.ItemChecked += listOrdEn_ItemChecked;
                listOrdEn.ItemCheck += listOrdEn_ItemCheck;
            }
        }
        private void listOrdEn_ItemCheck(object sender, ItemCheckEventArgs e)
        {
            ...
            Point locaPoint = listOrdEn.PointToClient(MousePosition);
            ListViewItem prevHoverdItem = listOrdEn.GetItemAt(locaPoint.X, locaPoint.Y);
            if (prevHoverdItem == null) return;
            if (prevHoverdItem != listOrdEn.Items[e.Index]) e.NewValue = CheckState.Checked;
        }
